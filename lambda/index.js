  ...
  async handle(handlerInput) {
    ...
    handlerInput.responseBuilder
        ...
        // get data from database     
                   await axios(config).then(response => {
                       for (const record in response.data.records) {
                             if (response.data.records[record].fields.Name ===  sessionAttributes.Name) { 
        ...
        if (sessionAttributes.classification_state !== "FINISHED" && sessionAttributes.classification_state !== "MEMORY") {
            ...
            const speakOutput = `Welcome back ${sessionAttributes.Name}, <break time="1s"/>  last time you didn't finished the classification part. We will start it again. `
            return handlerInput.responseBuilder
        ...
        if( sessionAttributes.id !== "Name not found" && sessionAttributes.game_state === "FINISHED" && sessionAttributes.classification_state === "FINISHED" ){  //sessionAttributes['first_time_flag'] === 0 
            const speakOutput = `Welcome back ${sessionAttributes['Name']}!`;
        ...
        if ( (sessionAttributes.game_state === "MEMORY" || sessionAttributes.classification_state === "MEMORY")  && sessionAttributes.flag_30_min === 1) {
            ...
            if (sessionAttributes.game_state === "MEMORY") {
                speakOutput = `<amazon:emotion name="excited" intensity="medium"> Welcome back ${sessionAttributes.Name}, <break time="1s"/>  one more effort. <break time="1s"/> Just tell me the words you remember and the training will be concluded </amazon:emotion>`;
            }
            if (sessionAttributes.classification_state ==="MEMORY") {
                speakOutput = `<amazon:emotion name="excited" intensity="medium"> Welcome back ${sessionAttributes.Name} <break time="1s"/> , tell me the words you remember and the classification part will be over. </amazon:emotion>`;
                sessionAttributes.score_attention = 0;
                sessionAttributes.score_orientation = 0;
                handlerInput.attributesManager.setSessionAttributes(sessionAttributes);
            }
...
const NoIntentHandler = {
     ...
     async handle(handlerInput) {
         ...
         if (sessionAttributes.classification_state === "START") {
             speakOutput = `No problem ${name}. <break time="1s"/> Come back later and open again the skill, if you change idea. `;
...
const IntentReflectorHandler = {
    ...
    handle(handlerInput) {
        ...
        if (sessionAttributes.game_state === "START" && intentName === "AnswerIntent") {
            speakOutput1 = `Sorry ${sessionAttributes.Name}, i don't understand what you just said. Would you like to play my games?.`;
        ...
        if (sessionAttributes.classification_state === "START" && intentName === "AnswerIntent") {
            speakOutput1 = `Sorry ${sessionAttributes.Name}, i don't understand what you just said. .`;
